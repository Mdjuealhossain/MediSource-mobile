                ...
                <div className="w-full">
                    <div className="px-3 relative mb-1 py-2 rounded border border-gary_700 w-full bg-paper">
                        ...
                        <div className="flex items-center gap-0.5">
                            ...
                            <p className="text-body2">
                                ...
                                <span className={`font-semibold pl-1 ${!isSpecial ? " hidden" : "inline"}`}>
                                    {type == "purchase" ? parseFloat((item.buying_price / item.total_qty || 0).toFixed(1)) : parseFloat((item.rate || 0).toFixed(1))}
                                    TK
                                </span>
                            </p>
                            <span className="px-1">|</span>
                            <p className="text-body2">
                                ...
                                <span className={`font-semibold pl-1 ${!isSpecial ? " hidden" : "inline"}`}>
                                    {type == "purchase" ? parseFloat((item.buying_price || 0).toFixed(1)) : parseFloat((item.total_amount || 0).toFixed(1))}
                                    TK
                                </span>
                            </p>
                        </div>
                    ...
                    {IsAdd && (
                        <form onSubmit={handleSubmit(onSubmit)} className=" bg-white z-50 rounded-md flex items-center justify-center">
                            ...
                            <button type="submit" className="text-body2 font-medium px-8 py-1 rounded-r text-white bg-success_main capitalize border">
                                Add
                            </button>
                        </form>
                    )}
